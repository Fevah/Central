...
inputs = {
  ...
  # Spot node group (prod only)
  spot_enabled        = true
  spot_instance_types = local.env.locals.spot_instance_types
  spot_desired        = local.env.locals.spot_desired
  spot_min            = local.env.locals.spot_min
  spot_max            = local.env.locals.spot_max
}
