...
{
    /// <summary>
    /// Removes all criteria that operate on properties that are not included in the validProperties list.
    /// </summary>
    public class RemoveInvalidPropertiesCriteriaVisitor : IClientCriteriaVisitor<CriteriaOperator>
    {
        ...
        public CriteriaOperator Start(CriteriaOperator criteriaOperator, List<string> validProperties)
        {
            // Abort if no criteriaOperator or validProperties were supplied
            if (ReferenceEquals(criteriaOperator, null) || validProperties == null || validProperties.Count == 0)
                return null;
        ...
        {
            // Return null if the OperandProperty does not exist in the validProperties list
            if (!_validProperties.Contains(theOperand.PropertyName))
                return null;
