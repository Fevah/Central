...
**Validation-rule catalog expansion** (37 batches from `cc4748f5f` through `6343bec52`): 54 → 152 rules. Batches 25-36 shipped the full lifecycle-resolves family on every FK in the `net.*` graph. Batch 37 added three more nullable-FK resolves on the BGP + provisioning pointers: `device.asn_allocation_resolves_active_when_set`, `server.asn_allocation_resolves_active_when_set`, `server.server_profile_resolves_active_when_set` (all Warning). Every FK in the `net.*` graph (mandatory or nullable) now has a lifecycle-resolves rule. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
