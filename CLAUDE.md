...
**Validation-rule catalog expansion — 212 rules** (57 batches from `cc4748f5f` through `76d089cb4`): 54 → 212 rules. Every FK in the `net.*` graph has a lifecycle-resolves rule (mandatory + nullable variants). Every `*_code` field has a whitespace-trim rule (hierarchy region→rack + entity device_code + pool pool_code + block block_code + catalog role_code/type_code/profile_code + shelf resource_key). Every `display_name` field on every entity + catalog + pool has a whitespace-trim rule. Uniqueness covers hostname / management_ip / serial_number. Structural integrity covers self-loop guards + hierarchy consistency. `runValidation` has server-side `category` filter so dashboards can run one category at a time. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
