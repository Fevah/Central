...
**Validation-rule catalog expansion** (41 batches from `cc4748f5f` through `da876f26a`): 54 → 164 rules. Batches 25-39 shipped the lifecycle-resolves family + initial data-quality advisories. Batch 40 added serial-number-uniqueness rules on device / server / module (Warning — duplicates break inventory sync). Batch 41 added three Advisory Info rules on reachability + readability: `device.last_ping_ok_when_active`, `server.last_ping_ok_when_active`, `link.description_set_when_active`. Every FK in the `net.*` graph has a lifecycle-resolves rule; the catalog now covers structural integrity + data-quality advisories. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
