...
**Validation-rule catalog expansion** (29 batches from `cc4748f5f` through `8e3c6100b`): 54 → 128 rules. Batches 25-29 shipped the full lifecycle-resolves family on every FK in the `net.*` graph: phys stack (room → floor, rack → room, port/module/loopback/aggregate_ethernet → device), server stack (server_nic → server), link stack (link_endpoint → link), device-role catalog (device → device_role), and numbering stack (vlan → vlan_block, subnet → ip_pool, mlag_domain → mlag_domain_pool, asn_block → asn_pool, vlan_block → vlan_pool, vlan → vlan_template). Every descendant of any `net.*` parent now surfaces as a violation the moment its parent goes non-Active. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
