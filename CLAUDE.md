...
**Validation-rule catalog expansion** (46 batches from `cc4748f5f` through `c2459726f`): 54 → 179 rules. Batches 25-44 shipped the lifecycle-resolves family + data-quality + config-correctness advisories. Batch 45 added three hygiene rules: `port.speed_mbps_set_when_active`, `server_nic.nic_index_in_range`, `device.hostname_no_leading_trailing_whitespace`. Batch 46 mirrored three more: `server.hostname_no_leading_trailing_whitespace`, `port.description_set_when_active`, `link.link_code_no_leading_trailing_whitespace`. Every FK in the `net.*` graph has a lifecycle-resolves rule; the catalog covers structural integrity + uniqueness + reachability + readability + config-correctness + whitespace hygiene. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
