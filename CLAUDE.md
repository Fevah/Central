...
**Validation-rule catalog expansion** (27 batches from `cc4748f5f` through `355cb5b1c`): 54 → 122 rules. Batches 25-27 shipped the full lifecycle-resolves family: `room.floor_resolves_active`, `rack.room_resolves_active`, `port.device_resolves_active`, `module.device_resolves_active`, `loopback.device_resolves_active`, `aggregate_ethernet.device_resolves_active`, `server_nic.server_resolves_active`, `link_endpoint.link_resolves_active`, `device.role_resolves_active`. Every descendant of region/site/building/floor/room/rack/device/server now surfaces as a violation the moment its parent goes non-Active. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
