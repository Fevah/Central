...
**Validation-rule catalog expansion — 203 rules** (54 batches from `cc4748f5f` through `2686b160d`): 54 → 203 rules (200-rule milestone hit in batch 53; batch 54 added three more pool_code whitespace mirrors). Every FK in the `net.*` graph has a lifecycle-resolves rule (mandatory + nullable variants). Every `*_code` field has a whitespace-trim rule (hierarchy: region/site/building/floor/room/rack + entity: device_code + pool: asn/vlan/ip/mlag pool_code). Every `display_name` field has a whitespace-trim rule. Uniqueness covers hostname / management_ip / serial_number. Structural integrity covers self-loop guards + hierarchy consistency. `runValidation` has server-side `category` filter so dashboards can run one category at a time. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
