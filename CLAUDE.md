...
**Validation-rule catalog expansion** (35 batches from `cc4748f5f` through `f6463b1dd`): 54 → 146 rules. Batches 25-34 shipped the full lifecycle-resolves family on every FK in the `net.*` graph. Batch 35 added three net.port internal-pointer resolves: `port.module_resolves_active_when_set`, `port.breakout_parent_resolves_active_when_set`, `port.aggregate_ethernet_resolves_active_when_set` (all Warning). Every FK in the `net.*` graph (mandatory or nullable) now has a lifecycle-resolves rule. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
