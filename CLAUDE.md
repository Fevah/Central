...
**Validation-rule catalog expansion** (42 batches from `cc4748f5f` through `98b18a0e0`): 54 → 167 rules. Batches 25-41 shipped the lifecycle-resolves family + data-quality advisories. Batch 42 added three more data-quality rules: `device/server.management_ip_unique_per_tenant_when_set` (Warning — probe/SSH collision catch) + `port.admin_up_false_on_active_status` (Info — planned-downtime / stale-import detection). Every FK in the `net.*` graph has a lifecycle-resolves rule; the catalog now covers structural integrity + uniqueness + reachability + readability advisories. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
