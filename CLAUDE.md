...
**Validation-rule catalog expansion — 224 rules** (61 batches from `cc4748f5f` through `f04958452`): 54 → 224 rules. Every FK in the `net.*` graph has a lifecycle-resolves rule (mandatory + nullable variants). Every `*_code` and `display_name` column in the `net.*` graph has a whitespace-trim rule (entity tables, pool tables, block tables, catalog tables, hierarchy tables + profile tables). Change-set titles have both not-empty and trim rules. Uniqueness covers hostname / management_ip / serial_number. Structural integrity covers self-loop guards + hierarchy consistency. `runValidation` has server-side `category` filter so dashboards can run one category at a time. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
