...
**Validation-rule catalog expansion** (36 batches from `cc4748f5f` through `9cd7baa27`): 54 → 149 rules. Batches 25-35 shipped the full lifecycle-resolves family on every FK in the `net.*` graph. Batch 36 added three more nullable-FK resolves on link_endpoint + server_nic: `link_endpoint.device_resolves_active_when_set`, `link_endpoint.ip_address_resolves_active_when_set`, `server_nic.ip_address_resolves_active_when_set` (all Warning). Every FK in the `net.*` graph (mandatory or nullable) now has a lifecycle-resolves rule. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
