...
**Validation-rule catalog expansion** (33 batches from `cc4748f5f` through `418ff2793`): 54 → 140 rules. Batches 25-32 shipped the full lifecycle-resolves family on every FK in the `net.*` graph (mandatory + nullable variants). Batch 31 added three non-lifecycle data-quality rules. Batch 33 finished the location-FK hierarchy resolves on device + server: `device.building_resolves_active_when_set`, `server.room_resolves_active_when_set`, `server.rack_resolves_active_when_set`. Every FK in the `net.*` graph (mandatory or nullable) now has a lifecycle-resolves rule. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
