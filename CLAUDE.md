...
**Validation-rule catalog expansion** (32 batches from `cc4748f5f` through `3734df521`): 54 → 137 rules. Batches 25-30 shipped the full lifecycle-resolves family on every FK in the `net.*` graph (mandatory + nullable variants). Batch 31 added three non-lifecycle data-quality rules. Batch 32 extended nullable-FK resolves with three more: `device.room_resolves_active_when_set`, `device.rack_resolves_active_when_set`, `subnet.parent_subnet_resolves_active_when_set`. Every FK in the `net.*` graph (mandatory or nullable) now has a lifecycle-resolves rule. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
