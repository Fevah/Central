...
**Validation-rule catalog expansion** (48 batches from `cc4748f5f` through `8c2ac5c61`): 54 → 185 rules. Batches 25-46 shipped the lifecycle-resolves family + data-quality + config-correctness + whitespace hygiene. Batch 47 added `ip_address.gateway_unique_per_subnet` (Warning — one Gateway per subnet) plus two more whitespace mirrors (subnet / vlan display_name). Batch 48 extended hygiene to server / link / ip_pool display_name. Every FK in the `net.*` graph has a lifecycle-resolves rule; display_name trim hygiene now covers device / server / subnet / vlan / link / ip_pool. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
