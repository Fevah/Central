...
**Web pages shipped** (`apps/web/src/app/modules/network/components/`): `network-search` (with facet chip bar + saved-view CRUD sidebar — create/rename/delete), `network-validation`, `network-scope-grants` (list + filter + create + delete + copy id + check permission dry-run), `network-hierarchy`, `network-pools`, `network-bulk` (validate-only dry-run), `network-devices` / `network-vlans` / `network-servers` / `network-links-grid` / `network-subnets` / `network-dhcp-relay` (thin grids for all six net.* entity types), `network-audit-timeline` (with clickable correlationId drill), `network-audit-search` (free-form filter + CSV/NDJSON export), `network-audit-stats` (DxChart trend + per-entity-type table + top-actors leaderboard in a two-column layout), `network-change-sets` (list) + `network-change-set-detail` (header + items grid with before/after JSON master-detail), `network-locks` (non-Open lock admin view across the 5 numbering tables), `network-naming-preview` (three-mode token-substitution dry-run), `network-render-history` (per-device render list + body viewer + line-set diff against previousRenderId + "Render now" trigger button), `network-pool-utilization` (ASN/VLAN/IP pool used-vs-capacity dashboard with coloured progress bars), `network-cli-flavors` (per-tenant enable + one-default CLI flavor admin). `NetworkingEngineService` is a typed thin wrapper over every Phase 10 endpoint. All pages lazy-loaded via `loadComponent` + gated by `moduleGuard('switches')`; per-page `moduleGuard('links'|'routing')` for fine-grained licensing.
...
**Validation-rule catalog expansion** (commits `cc4748f5f` + `90d931eed` + `bbd80fab9` + `a5b6e55fa` + `209306ed9` + `93fd3bbd0` + `a3d8a9d98`): 54 → 69 rules. Latest batch added `rendered_config.chain_integrity`, `scope_grant.no_duplicate_tuple`, `change_set.applied_has_no_pending_items`. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
