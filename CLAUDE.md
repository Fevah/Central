...
**Validation-rule catalog expansion** (34 batches from `cc4748f5f` through `6ea7a8639`): 54 → 143 rules. Batches 25-33 shipped the full lifecycle-resolves family on every FK in the `net.*` graph (mandatory + nullable variants). Batch 34 added link-type resolves + advisory: `link.link_type_resolves_active` (Error), `link.building_resolves_active_when_set` (Warning), `server.management_ip_set_when_active` (Advisory — parallel to the existing device.management_ip_for_active rule). Every FK in the `net.*` graph (mandatory or nullable) now has a lifecycle-resolves rule. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
