...
**Validation-rule catalog expansion** (39 batches from `cc4748f5f` through `fabff188e`): 54 → 158 rules. Batches 25-37 shipped the full lifecycle-resolves family on every FK in the `net.*` graph. Batch 38 closed the remaining nullable-FK gaps (server loopback IP, link-endpoint port, server building). Batch 39 mixed one lifecycle-resolves sibling with two Advisory data-quality rules: `server_nic.target_port_resolves_active_when_set` (Warning), `rack.uheight_within_reason` (Info — flags u_height > 60), `device.firmware_version_set_when_active` (Info — required for upgrade + CVE tracking). Every FK in the `net.*` graph (mandatory or nullable) has a lifecycle-resolves rule. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
