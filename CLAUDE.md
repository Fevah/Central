...
**Validation-rule catalog expansion** (51 batches from `cc4748f5f` through `4f9a84229`): 54 → 194 rules. Batches 25-49 shipped the lifecycle-resolves family + data-quality + config-correctness + whitespace hygiene + structural integrity. Batch 50 added `server.rack_implies_room` + mlag_domain display_name + rack_code whitespace mirrors. Batch 51 completed the hierarchy-code whitespace family (building / floor / room codes). `runValidation` has a server-side `category` filter so dashboards can run just one category. Every FK in the `net.*` graph has a lifecycle-resolves rule. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
