...
**Validation-rule catalog expansion** (44 batches from `cc4748f5f` through `8f1d7f2e1`): 54 → 173 rules. Batches 25-43 shipped the lifecycle-resolves family + data-quality advisories. Batch 44 added three config-correctness rules on L2 / LAG / change-set integrity: `port.native_vlan_requires_access_or_trunk` (Warning), `aggregate_ethernet.member_count_meets_min_links` (Warning — LAG won't come up), `change_set_item.expected_version_set_for_update` (Info — stale-write guard advisory). Every FK in the `net.*` graph has a lifecycle-resolves rule; the catalog covers structural integrity + uniqueness + reachability + readability + config-correctness + change-set integrity. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
