...
**Web pages shipped** (`apps/web/src/app/modules/network/components/`): `network-search` (with facet chip bar + saved-view CRUD sidebar — create/rename/delete), `network-validation`, `network-scope-grants` (list + filter + create + delete + copy id + check permission dry-run), `network-hierarchy`, `network-pools`, `network-bulk` (validate-only dry-run), `network-devices` / `network-vlans` / `network-servers` / `network-links-grid` / `network-subnets` / `network-dhcp-relay` (thin grids for all six net.* entity types; dhcp-relay has full create/edit/delete), `network-device-detail` (tabbed Summary/Audit/Renders page for one net.device row with "Render now" trigger inside the Renders tab), `network-audit-timeline` (with clickable correlationId drill), `network-audit-search` (free-form filter + CSV/NDJSON export), `network-audit-stats` (DxChart trend + per-entity-type table + top-actors leaderboard in a two-column layout), `network-change-sets` (list) + `network-change-set-detail` (header + items grid with before/after JSON master-detail + Submit/Apply/Cancel action bar), `network-locks` (non-Open lock admin view across the 5 numbering tables), `network-naming-preview` (three-mode token-substitution dry-run), `network-render-history` (per-device render list + body viewer + line-set diff against previousRenderId + "Render now" trigger button), `network-render-pack` (Building/Site/Region turn-up pack trigger with summary cards + per-device error grid), `network-pool-utilization` (ASN/VLAN/IP pool used-vs-capacity dashboard with coloured progress bars), `network-cli-flavors` (per-tenant enable + one-default CLI flavor admin). `NetworkingEngineService` is a typed thin wrapper over every Phase 10 endpoint. All pages lazy-loaded via `loadComponent` + gated by `moduleGuard('switches')`; per-page `moduleGuard('links'|'routing')` for fine-grained licensing.
