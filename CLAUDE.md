...
**Validation-rule catalog expansion — 200 rule milestone** (53 batches from `cc4748f5f` through `8745b8ca3`): 54 → 200 rules. Every FK in the `net.*` graph has a lifecycle-resolves rule (mandatory + nullable variants). Every `*_code` field (hierarchy + rack + device) has a whitespace-trim rule. Every `display_name` field on the six main entities + all four pool families has a whitespace-trim rule. Uniqueness rules cover hostname / management_ip / serial_number on device + server. Structural integrity covers self-loop guards (port breakout, subnet parent) + hierarchy consistency (rack_implies_room). Reachability + readability advisories catch Active-but-unpingable + missing descriptions. `runValidation` has a server-side `category` filter so dashboards can run just one category. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
