...
**Validation-rule catalog expansion** (20 batches of three rules from `cc4748f5f` through `dd3f10051`): 54 → 99 rules. Latest batches added hierarchy uniqueness (floor/room/rack code-per-parent), VRRP peer checks, hierarchy + profile display_name guards, module slot uniqueness, MSTP step presence, shelf key integrity, MLAG scope resolution, change-set-item entity_id requirement, and port speed sanity. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
...
**Supporting-entity thin lists + grids** (commits `dcb3ff09b` + `78c0054e5` + `f08d71b26` + `56a7e025d`): `/api/net/mlag-domains`, `/api/net/modules`, `/api/net/mstp-rules`, `/api/net/reservation-shelf`. Each has a tenant-wide web grid at the matching route with amber row-tints on data-quality edge cases (AE members < min_links, MSTP rule with 0 steps, shelf entries past cooldown). Backs operator "is anything wrong with the fabric?" scans at a glance.
