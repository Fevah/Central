...
**Validation-rule catalog expansion** (31 batches from `cc4748f5f` through `8135ed68d`): 54 → 134 rules. Batches 25-30 shipped the full lifecycle-resolves family on every FK in the `net.*` graph (both mandatory-FK and nullable-FK cases): phys stack, server stack, link stack, device-role catalog, numbering stack, and the `_when_set` Warning variants for server_nic.vlan/subnet + link_endpoint.vlan. Batch 31 added three non-lifecycle data-quality rules: `vlan_template.default_unique_per_tenant` (Error), `port.interface_name_starts_with_prefix` (Warning), `device.hardware_model_set_when_active` (Advisory/Warning). Every descendant of any `net.*` parent now surfaces as a violation the moment its parent goes non-Active. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
