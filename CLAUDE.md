...
**Validation-rule catalog expansion** (26 batches from `cc4748f5f` through `4db6df3bb`): 54 → 119 rules. Batches 25 + 26 added the full "device-child lifecycle resolves" family: `room.floor_resolves_active`, `rack.room_resolves_active`, `port.device_resolves_active`, `module.device_resolves_active`, `loopback.device_resolves_active`, `aggregate_ethernet.device_resolves_active`. Every descendant of region/site/building/floor/room/rack/device now surfaces as a violation the moment its parent goes non-Active. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
