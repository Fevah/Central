...
**Validation-rule catalog expansion** (49 batches from `cc4748f5f` through `701e4b16c`): 54 → 188 rules. Batches 25-47 shipped the lifecycle-resolves family + data-quality + config-correctness + whitespace hygiene. Batch 48 extended hygiene to server / link / ip_pool display_name. Batch 49 added three structural-integrity rules: `port.breakout_parent_not_self_loop` (Error), `subnet.parent_subnet_not_self_loop` (Error), `device.rack_implies_room` (Warning — hierarchy consistency). `runValidation` also gained a server-side `category` filter so dashboards can run just one category (Integrity / Consistency / Safety / Advisory) without materialising the per-code list. Every FK in the `net.*` graph has a lifecycle-resolves rule. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
