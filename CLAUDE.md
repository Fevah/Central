...
25 projects. .NET 10 / PG 18.3 / Npgsql 10.0.2 / DX 25.2 / Svg.NET 3.4.7 / Elsa 3.5.3. 54 API endpoint files. 109 DB migrations (001-109). 0 build errors. **2,781 tests** across ~190 classes (2026-04-20 — module-update system Phases 1-5 shipped: versioning, catalog, server-side distribution, ALC hot-swap kernel, SignalR fan-out, desktop wiring. Engine-level audits (65) + versioning (18) + storage (11) + ALC kernel (12) + safety lint (1) + manager dispatch (11) + downloader adapter (5)).
...
**Validation-rule catalog expansion — 234 rules** (65 batches from `cc4748f5f` through the 2026-04-20 catalog-display + pool-display + hierarchy-code not_empty sweep + pool_code uniqueness): 54 → 234 rules. Every FK in the `net.*` graph has a lifecycle-resolves rule (mandatory + nullable variants). Every `*_code` and `display_name` column in the `net.*` graph has a whitespace-trim rule (entity tables, pool tables, block tables, catalog tables, hierarchy tables + profile tables). Change-set titles have both not-empty and trim rules. Uniqueness covers hostname / management_ip / serial_number. Structural integrity covers self-loop guards + hierarchy consistency. `runValidation` has server-side `category` filter so dashboards can run one category at a time. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
...
**Twenty-fifth wave — /network/my-changesets + batch 44** (commits `0a026d28e` → `8f1d7f2e1`): five-slice arc: (1) Engine: `listChangeSets` gains an optional `requestedByUserId` narrower — same pattern as the `actorUserId` narrower on `listAudit`. (2) Web: new `/network/my-changesets` page — third leg of the self-serve trio (my-activity / my-grants / my-changesets); resolves caller via whoami + narrows by requestedByUserId. Overview quick-link chip bar adds a "My change sets" entry. (3) C# ApiClient: `ListChangeSetsAsync` gains `requestedByUserId` param + new `ListMyChangeSetsAsync` convenience chaining `WhoAmIAsync`. (4) Validation batch 44 (170 → 173) — config-correctness rules: `port.native_vlan_requires_access_or_trunk` (Warning), `aggregate_ethernet.member_count_meets_min_links` (Warning), `change_set_item.expected_version_set_for_update` (Info, Advisory). (5) Docs refresh.

**Twenty-sixth wave — validation batches 45+46 + category filter + pending-approvals panel** (commits `fd3042d67` → `83a2d63b6`): five-slice arc: (1) Validation batch 45 (173 → 176) — port/NIC/hostname hygiene: `port.speed_mbps_set_when_active`, `server_nic.nic_index_in_range`, `device.hostname_no_leading_trailing_whitespace`. (2) Web: validation page gains a Category quick-filter bar (All / Integrity / Consistency / Safety / Advisory) — filters via a ruleCode → category map fetched from `/api/net/validation/rules` on page load; silent-fail keeps the bar hidden if the catalog call errors. New `listValidationRules` service method + `ResolvedRule` interface. (3) Validation batch 46 (176 → 179) — mirrors: `server.hostname_no_leading_trailing_whitespace`, `port.description_set_when_active`, `link.link_code_no_leading_trailing_whitespace`. (4) Web: overview page gains a "Pending approvals" mini-panel — top 10 Submitted change-sets, click-to-detail drill, hidden when empty. (5) Docs refresh.

**Twenty-seventh wave — batches 47+48 + change-set rollup + ApiClient status-summary** (commits `946c3321e` → `7fa23983f`): five-slice arc: (1) Validation batch 47 (179 → 182) — `ip_address.gateway_unique_per_subnet` (Warning) + subnet / vlan display_name whitespace hygiene. (2) Web: change-set detail gains an item-action rollup pill bar above the Items grid (Create / Update / Rename / Delete counts with colour coding; hidden when empty). (3) Validation batch 48 (182 → 185) — server / link / ip_pool display_name whitespace hygiene mirrors. (4) C# ApiClient: `ChangeSetStatusSummaryAsync(organizationId, ct)` + `ChangeSetStatusCountDto` — mirrors the web wrapper shipped in wave 23. (5) Docs refresh.

**Twenty-eighth wave — validation category runner + batch 49** (commits `86cc9acce` → `701e4b16c`): five-slice arc: (1) Engine: `POST /api/net/validation/run` accepts an optional `category` body field — runs only rules where RuleMeta.category matches. Combines with `ruleCode` (both AND-filter). (2) Web: validation page toolbar gains four "Run Integrity / Consistency / Safety / Advisory" buttons alongside "Run all rules"; clicking one auto-activates the matching client-side category filter so the grid shows the correct subset instead of stale broader-run rows. (3) C# ApiClient: `RunValidationAsync` gains an optional `category` param. (4) Validation batch 49 (185 → 188) — structural integrity: `port.breakout_parent_not_self_loop` (Error), `subnet.parent_subnet_not_self_loop` (Error), `device.rack_implies_room` (Warning). (5) Docs refresh.

**Twenty-ninth wave — batches 50+51 + rules catalog browser** (commits `aa5bdaa32` → `128818742`): five-slice arc: (1) Validation batch 50 (188 → 191) — `server.rack_implies_room` + mlag_domain display_name + rack_code whitespace mirrors. (2) Web: new `/network/validation-rules` read-only grid browses the full rule catalog with tenant-effective severity + enabled state + override badge; grouped by Category. (3) Validation batch 51 (191 → 194) — hierarchy-code whitespace: building / floor / room codes. Completes the trim-check family on every *_code field. (4) Web: overview quick-links chip bar gains a "Rules catalog" entry. (5) Docs refresh.

**Thirtieth wave — 200-rule milestone + catalog-to-validation drill** (commits `0f867509a` → `8745b8ca3`): five-slice arc: (1) Validation batch 52 (194 → 197) — site / region / device_code whitespace mirrors. (2) Web: rules-catalog row double-click navigates to `/network/validation?ruleCode=…`. (3) Validation page reads the `ruleCode` query param on ngOnInit + auto-runs that single rule via a new `runSingle()` helper; summary line is specialised ("foo.bar — clean." or the usual run-summary). (4) Validation batch 53 (197 → 200 — milestone) — pool-name whitespace: asn_pool / vlan_pool / mlag_domain_pool. (5) Docs refresh.

**Thirty-first wave — search min-length guard + batch 54** (commits `c6e6572b2` → `0ec91c9a4`): five-slice arc: (1) Engine: `/api/net/search` + `/api/net/search/facets` short-circuit to [] when the trimmed query is shorter than `SEARCH_MIN_QUERY_LEN` (2). Saves wasted scans on 1-char queries that would match half the catalog. (2) Web: `/network/search` renders a "type at least 2 characters" amber hint when the query is exactly 1 char so operators see why Search returns nothing. (3) Validation batch 54 (200 → 203) — pool_code whitespace: asn_pool / vlan_pool / ip_pool (mlag_domain_pool's pool_code sits on `pool_code` ... completes the pool_code trim family). (4) C# ApiClient: `GlobalSearchAsync` gains a client-side guard matching the engine's `SEARCH_MIN_QUERY_LEN` — returns empty without the round trip + exposes `SearchMinQueryLength` const for UI feedback. (5) Docs refresh.

**Thirty-second wave — batches 55-57 + reset filters button** (commits `d6c1e524b` → `76d089cb4`): five-slice arc: (1) Validation batch 55 (203 → 206) — catalog-code whitespace: `device_role.role_code`, `link_type.type_code`, `server_profile.profile_code`. (2) Web: validation page gains a "Reset filters" button on the Category bar that clears severity + category to All in one click. Only renders when a filter is active. (3) Validation batch 56 (206 → 209) — `asn_block.block_code`, `vlan_block.block_code`, `reservation_shelf.resource_key` whitespace. (4) Validation batch 57 (209 → 212) — catalog-display whitespace: `device_role.display_name`, `link_type.display_name`, `server_profile.display_name`. Every display_name + *_code field in the net.* graph now has a trim check. (5) Docs refresh.

**Thirty-third wave — lifecycle summary + batch 58** (commits `e338ce5c9` → `8c0bd509b`): five-slice arc: (1) Engine: new `GET /api/net/tenant/lifecycle-summary` — per-entity-type × per-status rollup via UNION ALL across Device / Server / Vlan / Subnet / Link tables. One round-trip instead of 5 parallel calls. (2) Web: overview dashboard gains a "Lifecycle state" table pivoting the rollup into one row per entity type with Planned / Active / Decommissioned / Other columns. Operators spot stranded decommissioned entities at a glance. (3) C# ApiClient: `TenantLifecycleSummaryAsync(organizationId, ct)` + `LifecycleBucketDto(EntityType, Status, Count)`. (4) Validation batch 58 (212 → 215) — change-set title hygiene + VLAN template code trim: `change_set.title_not_empty` (Error), `change_set.title_no_leading_trailing_whitespace` (Warning), `vlan_template.template_code_no_leading_trailing_whitespace` (Warning). (5) Docs refresh.

**Thirty-fourth wave — batches 59-61 + rules-catalog category bar** (commits `fcd1abb67` → `f04958452`): five-slice arc: (1) Validation batch 59 (215 → 218) — hierarchy display_name whitespace: region / site / building. (2) Web: /network/validation-rules gains a colour-coded category summary pill bar showing rule count + override count per category (Integrity red, Consistency blue, Safety green, Advisory gray). (3) Validation batch 60 (218 → 221) — profile display_name whitespace: site_profile / building_profile + floor display_name (nullable, only fires when set). (4) Validation batch 61 (221 → 224) — final display_name trim sweep: floor_profile / vlan_template + device display_name (nullable). Every display_name + *_code in the net.* graph now has a trim check. (5) Docs refresh.

**Thirty-fifth wave — batches 62-64 not_empty sweep** (2026-04-20): five-slice arc: (1) Validation batch 62 — catalog display_name not_empty: `link_type.display_name_not_empty`, `server_profile.display_name_not_empty`, `building_profile.display_name_not_empty` (completes the *-type + profile display_name not_empty family). (2) Validation batch 63 — pool display_name not_empty: `asn_pool.display_name_not_empty`, `vlan_pool.display_name_not_empty`, `ip_pool.display_name_not_empty`. (3) Validation batch 64 — last-pool + hierarchy code not_empty: `mlag_domain_pool.display_name_not_empty` (closes pool not_empty family across ASN/VLAN/IP/MLAG), `floor.floor_code_not_empty`, `room.room_code_not_empty` (both rely on *_code as primary label). Catalog now at 231 rules (reconciled an off-by-two in earlier wave counts; the live count is authoritative via `grep -c "^    RuleMeta {"`). (4) `dispatcher_has_arm_for_every_catalog_rule` + full crate test suite green (298 / 298). (5) Docs refresh.

**Thirty-sixth wave — batch 65 pool_code uniqueness + full-suite discipline** (2026-04-20): five-slice arc: (1) Full-suite baseline — 2,714 / 2,723 .NET pass (9 known ServiceHealthTests infra probes fail when K8s VMs are down; reconciled as known-acceptable in feedback_full_test_suite_each_roll.md); 298 / 298 Rust pass. Memory rule saved: "run full suite every roll, not filtered subsets" after the user pushed back on filtered-only runs. (2) Validation batch 65 — pool_code uniqueness: `asn_pool.pool_code_unique_per_tenant`, `vlan_pool.pool_code_unique_per_tenant`, `ip_pool.pool_code_unique_per_tenant` (catalog 231 → 234). GROUP BY pool_code HAVING COUNT(*) > 1 pattern parallel to `device_role.unique_role_code_per_tenant`; sanity check that the DB's UNIQUE index is intact. (3) Full Rust suite green after batch 65. (4) Full .NET suite green after batch 65 (same 2,714 / 9 baseline). (5) Docs refresh.

**Thirty-seventh wave — module-update system design** (2026-04-20): five-slice arc: (1) Reviewed current module loader honestly — static Bootstrapper + lightweight `ModuleLoader.LoadPluginAssemblies` scanning `./plugins`; full-app UpdateManager only (no per-module swap); SignalR plumbing present but no ModuleUpdated message; `central_platform.client_versions` + `tenant_version_policy` tables exist for full-client releases but no module-level version tracking. (2) `docs/MODULE_UPDATE_SYSTEM.md` — six-phase buildout with three release kinds: HotSwap (silent, module-internal), SoftReload (banner + reload-now button), FullRestart (scheduled-restart installer). Engine interfaces + message-bus messages + permission codes are append-only by rule so most changes ship as HotSwap. (3) Memory rule `feedback_release_classification.md` — default HotSwap, escalate only when forced; designing a change for `FullRestart` is a trigger to stop + reshape. (4) Caught + fixed a parallel-test flake — `PanelMessageBusTests` raced `AllModulesRibbonAuditTests.btn.OnClick()` across xUnit collections; both now share `[Collection("RibbonAudit")]`. Full-suite baseline held at 2,714 / 9 / 298. (5) Docs refresh.

**Thirty-eighth wave — Phase 1 of the module-update system** (2026-04-20): five-slice arc: (1) `IModule.Version` + `IModule.EngineContractVersion` default interface implementations (append-only change; every existing module picks them up with zero code change — Version reads from AssemblyInformationalVersion, EngineContractVersion defaults to 1). `EngineContract.CurrentVersion = 1` host-side constant. New engine audit class `AllModulesVersioningAuditTests` (18 tests) asserts every module's Version is non-empty + EngineContractVersion is positive + doesn't exceed the host's current contract. (2) Migration 109 — `central_platform.module_catalog` gains `current_version` + `current_version_updated_at` columns; new `central_platform.module_versions` table (module_code + version + change_kind CHECK constraint + min_engine_contract + nullable blob_url/sha256/size for Phase 2 population + yank metadata). Seeds 1.0.0 HotSwap for every existing catalog row. (3) `/api/modules/catalog` + `/api/modules/{code}/versions` endpoints + `ModuleCatalogClient` typed wrapper on `libs/api-client`. (4) Full suites: 2,732 / 2,741 .NET (+18 from versioning audits; same 9 infra probes); 298 / 298 Rust. (5) Docs refresh.

**Thirty-ninth wave — Phase 2 of the module-update system (server-side distribution)** (2026-04-20): five-slice arc: (1) `IModuleBlobStorage` interface + `FilesystemModuleBlobStorage` adapter under `services/api/Services/` — writes blobs to `{root}/{moduleCode}/{version}/module.dll`, validated path segments (traversal rejection + Path.GetFullPath re-check against the root), idempotent on delete + empty-dir cleanup. DI-registered from `CENTRAL_MODULE_STORAGE_ROOT` env var (defaults to `./module-storage/`). MinIO adapter for multi-replica prod will implement the same interface — don't couple endpoints to the filesystem adapter directly. (2) `POST /api/modules/publish` (auth-gated) — multipart form with moduleCode / version / changeKind / minEngineContract / releaseNotes / setAsCurrent / dll file. SHA-256 + sizeBytes computed server-side from the upload stream (never trusted from the client). Single transaction inserts `module_versions` row + bumps `module_catalog.current_version` + rolls back + cleans blob on failure. Duplicate (23505) returns 409 Conflict; unknown moduleCode rolls back + deletes the uploaded blob. (3) `GET /api/modules/{code}/{version}/manifest` — cheap metadata lookup (no bytes). `GET /api/modules/{code}/{version}/dll` — streams bytes from the blob store; yanked versions return 410 Gone; missing bytes return 404 with an honest error explaining the row predates Phase 2. (4) `ModuleCatalogClient` gains `GetManifestAsync` / `DownloadDllAsync` / `PublishAsync` wrappers + three DTOs (`ModuleManifestDto`, `ModulePublishResultDto`, existing Version/Catalog DTOs). `FilesystemModuleBlobStorageTests` (11 tests): round-trip, missing-read, exists-tracks-write-delete, overwrite, path-traversal rejection (4 inline data), empty-root rejection, auto-create root, empty-dir cleanup on delete. (5) Full suites: 2,743 / 2,752 .NET (+11 storage tests; same 9 infra probes) + 298 / 298 Rust. Docs refresh.

**Fortieth wave — Phase 3 kernel: AssemblyLoadContext hot-swap primitives** (2026-04-20): five-slice arc: (1) Three new lifecycle messages on `PanelMessageBus` (additive — handlers that don't care about hot-swap don't subscribe): `ModuleReloadingMessage(code, fromVersion, toVersion)` fires before unload so panels close + release refs, `ModuleReloadedMessage(code, fromVersion, toVersion)` fires after the new DLL loads + registers, `ModuleLoadFailedMessage(code, attemptedVersion, reason, exception?)` fires on any failure in the chain. (2) `CollectibleModuleLoadContext` + `ModuleHost` under `libs/engine/Modules/` — new files, existing `ModuleLoader` untouched so the live path stays stable. Context is per-module + collectible so `Unload()` can actually free the old version. Host owns Load/Unload/Reload with SHA-256 pre-validation, `EngineContractVersion` compatibility gate, single-module-instance-at-a-time state machine, honest WPF rooting caveats documented in the class XML (ResourceDictionary cache, `EventManager.RegisterClassHandler` statics, XAML pack:// URIs, event-subscription GC roots). Emits lifecycle messages on every transition. (3) `AllModulesHotSwapSafetyAuditTests` — engine-level lint that scans every `.cs` under `modules/` for the two most common GC-rooting patterns (`EventManager.RegisterClassHandler` + `Application.Current.Resources.MergedDictionaries.Add`); zero offenders today. (4) `ModuleHostTests` (12 tests) — Load populates LiveModule + Version, Unload clears state + publishes weak ref, already-loaded rejects Load, empty bytes rejected, SHA-256 mismatch rejected without polluting state, empty module-code rejected, Unload idempotent when not loaded, Reload publishes ModuleReloading + ModuleReloaded, Reload with bad SHA publishes ModuleLoadFailed + ends unloaded, CollectibleContext rejects empty code, LoadFromBytes populates the assembly, GC forced collection wires WeakReference (lenient assertion — no CI flakes). (5) Full suites: 2,756 / 2,765 .NET (+13 from the kernel tests; same 9 infra probes); 298 / 298 Rust. Docs refresh.

**Forty-first wave — Phase 4: SignalR fan-out + ModuleHostManager dispatch** (2026-04-20): five-slice arc: (1) `NotificationHub` — `POST /api/modules/publish` now broadcasts `ModuleUpdated` to `Clients.All` on successful publish (only when `setAsCurrent=true`; publishing a version without promoting it stays quiet). Read of `current_version` before the bump so the payload can include `fromVersion` → `toVersion`. SignalR send is best-effort; fan-out failure doesn't roll back the publish (catalog is source of truth, clients discover via poll on next boot). (2) `SignalRClient` — new `ModuleUpdated` event + `ModuleUpdatedPayload` record (moduleCode / fromVersion / toVersion / changeKind / minEngineContract / sha256 / sizeBytes / publishedAt). Matches server payload shape 1:1. (3) `ModuleHostManager` + `IModuleUpdatePolicy` + `IModuleDllDownloader` + `DefaultModuleUpdatePolicy` + `ModuleUpdateNotification` record in `libs/engine/Modules/`. Manager owns N `ModuleHost` instances keyed by moduleCode, serialises updates per-module via SemaphoreSlim (different modules run in parallel; same module never overlaps), filters notifications to only registered hosts (unlicensed modules silently dropped), skips same-version no-ops, routes per changeKind: HotSwap (download + Reload silently), SoftReload (policy gets applyAction callback — wait for user click), FullRestart (policy handles; host untouched since process restarts). Failures route to `IModuleUpdatePolicy.HandleFailureAsync` without throwing out of `HandleNotificationAsync`. (4) `ModuleHostManagerTests` (11 tests) — HotSwap downloads + reloads + notifies, SoftReload doesn't download until policy applies, SoftReload with apply callback downloads + reloads, FullRestart doesn't touch host, unknown changeKind reports failure, unknown module silently dropped, same-version no-op skipped, null downloader response reports failure, SHA-256 mismatch reports failure + leaves host unloaded, RegisterHost rejects null, ctor rejects null deps. (5) Full suites: 2,767 / 2,776 .NET (+11 manager tests; same 9 infra probes) + 298 / 298 Rust. Docs refresh.

**Forty-second wave — Phase 5: desktop wiring (pipeline connected, hot-swap not yet)** (2026-04-20): five-slice arc: (1) `ApiClientModuleDllDownloader` in `libs/api-client` — adapter implementing `IModuleDllDownloader` by delegating to `ModuleCatalogClient.DownloadDllAsync`. Returns null on 404 / 410 (yanked); other failures propagate. (2) `WpfModuleUpdatePolicy` in `apps/desktop/Services/` — honest Phase 5 UX using `NotificationService` toasts for every changeKind. HotSwap-applied → Success toast; SoftReload-available → Info toast with "restart to apply" hint (conservative: does NOT auto-apply — richer banner with "Reload now" button lands in Phase 5b); FullRestart → Warning toast; failure → Error toast with reason. (3) `ModuleUpdateStartup` static helper in `apps/desktop/Services/` — `Initialize(apiBaseUrl)` constructs `ModuleCatalogClient` + `ApiClientModuleDllDownloader` + `WpfModuleUpdatePolicy` + `ModuleHostManager`, idempotent on same URL, rebuilds on URL change. Wired into `App.xaml.cs` right after login; also into the two `MainWindow.xaml.cs` reconnect paths. `ConnectivityManager.ConnectSignalRAsync` subscribes `_signalR.ModuleUpdated` → routes through `ModuleUpdateStartup.ManagerOrNull.HandleNotificationAsync`. Pipeline end-to-end: server publishes → SignalR fan-out → desktop routes → manager dispatches → policy toasts. No hosts registered yet, so silent drops until Phase 5b migrates loaded modules to the ALC path. (4) `ApiClientModuleDllDownloaderTests` (5 tests) — success round-trip, 404 → null, 410 Gone → null, 5xx → throws, null catalog rejected. (5) Full suites: 2,772 / 2,781 .NET (+5 from downloader tests; same 9 infra probes) + 298 / 298 Rust. Docs refresh.
