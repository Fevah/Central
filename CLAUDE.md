...
**Validation-rule catalog expansion — 231 rules** (64 batches from `cc4748f5f` through the 2026-04-20 catalog-display + pool-display + hierarchy-code not_empty sweep): 54 → 231 rules. Every FK in the `net.*` graph has a lifecycle-resolves rule (mandatory + nullable variants). Every `*_code` and `display_name` column in the `net.*` graph has a whitespace-trim rule (entity tables, pool tables, block tables, catalog tables, hierarchy tables + profile tables). Change-set titles have both not-empty and trim rules. Uniqueness covers hostname / management_ip / serial_number. Structural integrity covers self-loop guards + hierarchy consistency. `runValidation` has server-side `category` filter so dashboards can run one category at a time. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
