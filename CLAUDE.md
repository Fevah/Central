...
**Validation-rule catalog expansion** (15 batches of three rules from `cc4748f5f` through `61ce354cf`): 54 → 84 rules. Latest three batches added `port.breakout_parent_on_same_device`, `port.aggregate_on_same_device`, `aggregate_ethernet.name_unique_per_device`, `change_set.submitted_has_items`, `reservation_shelf.cooldown_respected`, `vlan_block.range_within_pool`. Guardrail test `dispatcher_has_arm_for_every_catalog_rule` updated each arc so a catalog row without a matching dispatcher arm fails CI.
