...
/// Used for: ManageEngine notifications, Entra SCIM events, GitHub/CI webhooks, etc.
/// </summary>
    ...
    public static RouteGroupBuilder MapWebhookEndpoints(this RouteGroupBuilder group)
    {
        ...
        group.MapPost("/{source}", async (string source, HttpContext ctx, DbConnectionFactory db, IHubContext<NotificationHub> hub) =>
        {
            ...
            try
            {
                ...
                {
                    // Mark as pending sync — the background job or manual trigger will pick it up
        ...
        group.MapGet("/", async (DbConnectionFactory db, int? limit) =>
        {
            await using var conn = await db.OpenConnectionAsync();
            await using var cmd = new Npgsql.NpgsqlCommand(
                $"SELECT id, source, received_at, processed FROM webhook_log ORDER BY received_at DESC LIMIT {Math.Min(limit ?? 50, 500)}", conn);
