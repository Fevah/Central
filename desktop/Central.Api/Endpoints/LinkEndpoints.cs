...
public static class LinkEndpoints
{
    public static RouteGroupBuilder MapLinkEndpoints(this RouteGroupBuilder group)
    {
        ...
        {
            return await UpdateRow("p2p_links", id, body, db);
        });

        group.MapPost("/p2p", async (Dictionary<string, object?> body, DbConnectionFactory db) =>
        {
            return await InsertRow("p2p_links", body, db);
        });
        ...
        {
            return await UpdateRow("b2b_links", id, body, db);
        });

        group.MapPost("/b2b", async (Dictionary<string, object?> body, DbConnectionFactory db) =>
        {
            return await InsertRow("b2b_links", body, db);
        });
        ...
        {
            return await UpdateRow("fw_links", id, body, db);
        });

        group.MapPost("/fw", async (Dictionary<string, object?> body, DbConnectionFactory db) =>
        {
            return await InsertRow("fw_links", body, db);
        });
    ...
    private static async Task<IResult> UpdateRow(string table, int id, Dictionary<string, object?> body, DbConnectionFactory db)
    {
        if (body.Count == 0)
    ...
    private static async Task<IResult> InsertRow(string table, Dictionary<string, object?> body, DbConnectionFactory db)
    {
        if (body.Count == 0)
