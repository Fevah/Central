...
public partial class DbRepository
{
    ...
    public async Task<List<Central.Engine.Models.TaskItem>> GetTasksAsync(int? projectId = null)
    {
        var list = new List<Central.Engine.Models.TaskItem>();
        await using var conn = await OpenConnectionAsync();
        ...
        await using var rdr = await cmd.ExecuteReaderAsync();
        while (await rdr.ReadAsync())
            list.Add(new Central.Engine.Models.TaskItem
            {
                Id = rdr.GetInt32(0),
    ...
    public async Task UpsertTaskAsync(Central.Engine.Models.TaskItem t)
    {
        await using var conn = await OpenConnectionAsync();
        if (t.Id == 0)
        {
    ...
    private static void AddTaskParams(NpgsqlCommand cmd, Central.Engine.Models.TaskItem t)
    {
