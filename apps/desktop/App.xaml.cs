...
public partial class App : System.Windows.Application
{
    ...
    protected override async void OnStartup(System.Windows.StartupEventArgs e)
    {
        ...
        try
        {
            ...
            var pgPassword = Environment.GetEnvironmentVariable("CENTRAL_PG_PASSWORD") ?? "central";
            Dsn = Environment.GetEnvironmentVariable("CENTRAL_DSN")
                ?? $"Host=127.0.0.1;Port=5432;Database=central;Username=central;Password={pgPassword};Timeout=5";
            if (Environment.GetEnvironmentVariable("CENTRAL_DSN") == null)
                Log("INFO: No CENTRAL_DSN env var — using localhost with CENTRAL_PG_PASSWORD (default: central).");
