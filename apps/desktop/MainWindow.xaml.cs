...
public partial class MainWindow
{
    ...
    public MainWindow()
    {
        ...
        VM.PropertyChanged += (_, e) =>
        {
            ...
            if (e.PropertyName == nameof(MainViewModel.IsPoolsPanelOpen))
            {
            ...
            if (e.PropertyName == nameof(MainViewModel.IsLocksPanelOpen))
            {
                ToggleDockPanel(LocksPanel, VM.IsLocksPanelOpen);
                if (VM.IsLocksPanelOpen) _ = LocksPanelView.ReloadAsync();
            }
            if (e.PropertyName == nameof(MainViewModel.IsBulkPanelOpen))
            {
                ToggleDockPanel(BulkPanel, VM.IsBulkPanelOpen);
    ...
    {
        var (_, activeView) = GetActiveGrid();
        var view = activeView ?? DeviceGridPanel?.View;
        if (view == null) return;

        var dlg = new Microsoft.Win32.SaveFileDialog
        {
            Filter   = "Excel Workbook (*.xlsx)|*.xlsx|PDF Document (*.pdf)|*.pdf|CSV (*.csv)|*.csv",
            FileName = $"Central_Export_{DateTime.Now:yyyyMMdd}"
        };
        if (dlg.ShowDialog() == true)
        {
            try
            {
                ...
                switch (ext)
                {
                    case ".xlsx": view.ExportToXlsx(dlg.FileName); break;
